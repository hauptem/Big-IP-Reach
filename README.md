...
- **The filter box** narrows the page as you type, matching device names, FQDNs, zones, hosts, and card titles. Terms separated by spaces are OR'd, so `mumbai cairo` shows both. `Alt+F` focuses the filter; `Alt+C` clears it; `Escape` also clears and returns to `All`.
- **The header** grows to as many button rows as needed and never overlaps the logo, title, or filter.

## The logo

The header image is embedded in the generated HTML as a placeholder. To replace it, open the HTML in a text editor and follow the `LOGO` comment near the top: convert a PNG to base64 and paste it into the `src` of the `<img>` tag. Rebuilding from the config restores the placeholder, so to make a logo permanent, make the same edit once inside `Build-Topology.ps1`.
